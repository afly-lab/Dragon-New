    ...
    <section id="audit-planner" className="py-24 md:py-32 bg-cream border-t border-ink/5">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          {/* Input + result card */}
          <div className="lg:col-span-7 bg-white border border-gold/20 shadow-sm p-8 rounded-sm shadow-xl shadow-ink/5">
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <input
                    ...
                    className="flex-1 bg-gray-50 border border-gray-200 rounded-sm py-4 px-6 text-sm text-ink outline-none focus:border-gold focus:ring-1 focus:ring-gold/30 transition-all"
                  />
                  <button
                    onClick={() => generateAudit()}
                    disabled={auditLoading}
                    className="bg-ink hover:bg-gold text-cream hover:text-ink py-4 px-8 font-mono text-xs uppercase tracking-widest font-bold rounded-sm transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-60"
                  >
                    {auditLoading ? (
            ...
            {auditResult && (
              <div id="audit-result-view" className="mt-12 pt-12 border-t border-ink/10 space-y-8 animate-fade-up">
                ...
                <div className="p-6 bg-blue-50/40 border-l-2 border-gold rounded-sm">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {(auditResult?.manufacturingClusters || []).map((cluster, i) => (
                      <div key={i} className="flex gap-4 p-5 bg-white border border-gray-100 rounded-sm hover:border-gold/50 transition-all">
                ...
                <div className="p-6 bg-ink text-cream rounded-sm border border-gold/20">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-widest text-gold">How Xinao Protects You</span>
                  </div>
                ...
                <a
                  href="#contact"
                  className="w-full text-center bg-gold hover:bg-ink text-ink hover:text-cream font-mono text-[10px] uppercase tracking-widest font-bold py-4 px-4 rounded-sm transition-all duration-300 block"
                >
